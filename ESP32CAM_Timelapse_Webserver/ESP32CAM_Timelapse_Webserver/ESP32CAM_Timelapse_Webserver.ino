...
static esp_err_t get_image(fs::FS &fs, const char *path)
{
    ...
    bool spiffs_ok = 0; // Boolean indicating if the picture has been saved

    do
    {
        // Save same photo to SPIFFS for webserver
        // Photo file name
        ...
        if (!spiffsFile)
        {
            Serial.println("Failed to open file in Spiffs writing mode");
        }
        else
        {
            spiffsFile.write(fb->buf, fb->len); // payload (image), payload length
            Serial.print("The picture has been saved in Spiffs in ");
        ...
        spiffs_ok = checkSpiffsPhoto(SPIFFS);
    } while (!spiffs_ok);

    return ESP_OK;
}

// Check if spiffs photo save was successful
bool checkSpiffsPhoto(fs::FS &fs)
{
